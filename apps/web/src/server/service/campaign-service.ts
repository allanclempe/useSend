...
// Lifted into `campaign-analytics-service` so `email-queue-service` can bump
// `Campaign.sent` at the SES handoff without importing this module, which
// imports it back. Re-exported so existing callers are unaffected.
export { updateCampaignAnalytics } from "./campaign-analytics-service";
