...
import type { EnabledAuthProviders } from "~/server/better-auth";
...
/**
 * The social buttons, in render order.
 *
 * Name and icon used to arrive from NextAuth's `getProviders()`, which the
 * login and signup pages fetched over the network on every render just to learn
 * two display strings. better-auth has no such endpoint and does not need one —
 * which providers exist is a build-time fact about our own `env`.
 */
...
export default function LoginPage({
  ...
  isSignup?: boolean;
  /** From `?inviteId=`, carried through sign-in so the invite is accepted after. */
  inviteId?: string;
  /** From `?error=`, where better-auth's OAuth callback puts its code. */
  error?: string;
}) {
  ...
  const enabledSocialProviders = SOCIAL_PROVIDERS.filter(
    (provider) => providers?.[provider.id] ?? false,
  );
        ...
        <div className="flex flex-col gap-8 mt-8 border p-8 rounded-lg shadow">
          ...
          {enabledSocialProviders.map((provider) => (
            ...
            >
              ...
              <span className="ml-4">
                {isSignup ? "Sign up with" : "Continue with"} {provider.name}
              </span>
            </Button>
          ))}
          {emailEnabled && (
