/**
 * The "send feedback" box in the sidebar footer, on TanStack Start (#9).
 *
 * Cloud only — the sidebar hides it on a self-hosted install, and
 * `functions/feedback.ts` refuses there as well, because a self-hosted install
 * has no founder to mail.
 */
...
import { send } from "~/server/functions/feedback";
...
export function FeedbackDialog({ trigger }: { trigger?: ReactNode }) {
  ...
  function onSubmit(values: z.infer<typeof FeedbackSchema>) {
    feedbackMutation.mutate({ data: { message: values.message.trim() } });
  }
