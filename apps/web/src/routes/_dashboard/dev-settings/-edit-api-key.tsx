...
/**
 * Rename an API key, or change which domain it may send from.
 *
 * Self-contained, like the other dialogs in this area: it owns the pencil
 * button that opens it. The list held an `editingId` and passed `open` down,
 * which is one piece of state per table rather than per row and made the row
 * responsible for a dialog it does not otherwise know about.
 *
 * The domain list is fetched here rather than warmed in the route loader: it
 * is only needed once someone opens this.
 */
export function EditApiKey({ apiKey }: { apiKey: ApiKeyRow }) {
  ...
  // One instance per row, mounted for as long as the row is; without this it
  // would reopen showing whatever was last typed into it.
  useEffect(() => {
    if (open) {
      form.reset({
        name: apiKey.name,
