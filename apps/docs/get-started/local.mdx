...
<Steps>
  ...
  <Step title="Neon Local">
  The dev database is [Neon Local](https://neon.com/docs/local/neon-local), a
  container that proxies to a real Neon branch rather than storing data on your
  machine. Create a Neon project, then fill in `NEON_PROJECT_ID`, `NEON_API_KEY`
  and `NEON_BRANCH_ID` in `.env`.

  <Note>
    `DATABASE_URL` stays as shipped in `.env.example`. `neon:npg` is the fixed
    local credential pair; the proxy swaps in the real branch credentials.
  </Note>
  </Step>
  <Step title="App secret">
  Use the following command to generate a key and add it under ```APP_SECRET```
...
We are using a local Postgres server through Docker. If you don't have Docker you can point `DATABASE_URL` at your own.
...
<Steps>
<Step title="Set up your PostgreSQL database in Environment Variables">
```bash
DATABASE_URL=""
```
</Step>
